...
def velocidade_atual():
    return min(VEL_INICIAL + frutas_comidas // 3, VEL_MAXIMA)


def fase_atual():
    return min(1 + frutas_comidas // 6, 5)


def gerar_fruta():
    global pos_fruta, fruta_atual
    livres = [(c, r) for c in range(COLS) for r in range(ROWS) if [c, r] not in corpo_cobra]
    if not livres:
        pos_fruta = None
        return
    pos_fruta = random.choice(livres)
    fruta_atual = random.choice(list(frutas.keys())) if frutas else None


def iniciar_partida():
    global corpo_cobra, direcao, proxima_direcao, score, frutas_comidas, fase, estado
    meio_coluna = COLS // 2
    meio_linha = ROWS // 2
...
def desenhar_fruta():
    if pos_fruta is None:
        return
    x, y = celula_para_pixel(*pos_fruta)
    centro = (x + CELL // 2, y + CELL // 2)
    if fruta_atual and fruta_atual in frutas:
        sprite = frutas[fruta_atual]
...
def desenhar_hud():
    ...
    desenhar_cartela(f"VEL {velocidade_atual()}", (LARGURA_TELA - 360, 122), COR_CREME)
...
while rodando:
        ...
        if evento.type == pygame.QUIT:
            ...
            if evento.key == pygame.K_ESCAPE:
                ...
                if evento.key in (pygame.K_LEFT, pygame.K_a):
                    ...
                    desejada = None
                if desejada and not eh_oposta(desejada, direcao):
                    proxima_direcao = desejada
    ...
    if estado == "jogando":
        ...
        if bateu_parede or bateu_corpo:
            ...
            if pos_fruta and tuple(nova_cabeca) == pos_fruta:
                frutas_comidas += 1
                score += 10
                if som_mordida:
                    som_mordida.play()
